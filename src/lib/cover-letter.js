...
- Currently Software Engineer at Magna International (Factory Solutions, since July 2025) — this is full-stack work, both backend AND frontend, not just backend: builds GoLang microservices for AMR bot automation (dispatch service, error handling/monitoring), AND independently designed + delivered an enterprise-grade RBAC-via-OIDC authentication system from scratch, including the Vue.js frontend with OIDC-token-based protected routes (gathered requirements across robotics/operations/security teams, architected the full auth/authorization flow). Also works on SmartPick, a forklift/tugger bot automation platform. Stack: GoLang, Vue.js, Docker, Kubernetes.
- Before that, at Cadera Infotech: built the Study Abroad section of CaderaEdu (React.js, Node.js/Express.js, MongoDB), including an admin/CRM dashboard and AI-powered workflows that automated ingestion and enrichment of college data.
...
- Never write "I was a Full-Stack Developer" or similar past-tense job-title framing for a previous company — it implies the candidate no longer does that kind of work, which is false and contradicts the opening "Full-Stack/AI Engineer" self-description. Describe past roles by the concrete work built (skills/tech), not by asserting a title that changed over time. When describing Magna, make sure BOTH the GoLang backend work AND the Vue.js frontend work come through — it's current, ongoing full-stack work, not backend-only.
